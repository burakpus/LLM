...
import { listTemplates, listSkillExamples } from '../../api/admin'
...
async function fetchSkillPrompt(id: string): Promise<string> {
  ...
  return text.length <= MAX_SKILL_CHARS ? text : text.slice(0, MAX_SKILL_CHARS) + '\n\n[...truncated]'
}
...
export default function InputBar({ onSend, onStop, onRegenerate, generating,
  fileContext, onFileContextConsumed, skills = [] }: Props) {
  ...
  // ── Skill selection (moved from Header) ───────────────────────────────────
  const setSkill = useCallback(async (id: string | null, name: string | null, collection?: string | null) => {
    let systemPrompt = ''
    let skillExamples: { user: string; assistant: string }[] = []
    if (id) {
      systemPrompt = await fetchSkillPrompt(id).catch(() => '')
      const rawEx = await listSkillExamples(id).catch(() => [])
      skillExamples = rawEx.map(e => ({ user: e.userMessage, assistant: e.assistantMessage }))
    }
    if (conv) store.updateConvSettings(conv.id, {
      skillId: id, skillName: name, skillCollection: collection ?? null,
      systemPrompt, skillExamples,
    })
  ...
  const activeSkillId   = settings?.skillId ?? store.activeSkillId
  const activeSkillName = settings?.skillName ?? store.activeSkillName
  const modLabel = activeSkillName
    ? (activeSkillName.length > 10 ? activeSkillName.slice(0, 9) + '…' : activeSkillName)
    : 'Mod'
    ...
    <div className="shrink-0 px-4 pt-2 pb-4 w-full" style={{ background: 'var(--bg)' }}>
      <div className="mx-auto" style={{ maxWidth: '760px' }}>
        ...
        <div className="mt-2 flex items-center gap-1.5">
          ...
          {/* 2. Mod dropdown (RAG + Skills) */}
          <div className="relative z-50">
            <button
              onClick={() => { setShowModDrop(o => !o); setShowEndDrop(false); setShowFmtDrop(false) }}
              className={`pill ${agentMode || activeSkillId ? 'active' : ''}`}
              style={activeSkillId ? { background: 'rgba(138,180,248,0.15)', color: 'var(--accent-hi)', border: '1px solid rgba(138,180,248,0.35)' } : {}}
            >
              <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
              {modLabel}
              {agentMode && <span className="text-[9px] ml-0.5 opacity-70">·RAG</span>}
              <svg className="w-2.5 h-2.5 ml-0.5 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {showModDrop && (
              <div className="absolute left-0 bottom-full mb-2 rounded-xl shadow-2xl overflow-hidden z-50"
                   style={{ background: 'var(--surface-hi)', border: '1px solid var(--border)', minWidth: 200, maxHeight: 360, overflowY: 'auto' }}>

                {/* RAG toggle */}
                <button
                  onClick={toggleAgent}
                  className="w-full flex items-center justify-between px-3 py-2.5 text-xs cursor-pointer transition"
                  style={{ color: agentMode ? 'var(--accent-hi)' : 'var(--text-2)', borderBottom: '1px solid var(--border)' }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--surface-2)'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}
                >
                  ...
                  <span className="w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center transition-colors"
                        style={{ borderColor: agentMode ? 'var(--accent)' : 'var(--mute)', background: agentMode ? 'var(--accent)' : 'transparent' }}>
                ...
                {skills.length > 0 && (
                  <>
                    ...
                    {/* No skill option */}
                    <button
                      onClick={() => setSkill(null, null)}
                      className="w-full flex items-center gap-2 px-3 py-2 text-xs cursor-pointer transition text-left"
                      style={{ borderBottom: '1px solid var(--border)',
                               color: !activeSkillId ? 'var(--accent-hi)' : 'var(--text-2)',
                               background: !activeSkillId ? 'rgba(138,180,248,0.1)' : 'transparent' }}
                      onMouseEnter={e => { if (activeSkillId) (e.currentTarget as HTMLElement).style.background = 'var(--surface-2)' }}
                      onMouseLeave={e => { if (activeSkillId) (e.currentTarget as HTMLElement).style.background = 'transparent' }}
                    >
                      {!activeSkillId
                        ? <span className="w-3 h-3 mr-1 text-[10px]">✓</span>
                        : <span className="w-3 h-3 mr-1" />}
                      <span style={{ color: 'var(--mute)' }}>Yok (varsayılan)</span>
                    </button>
                    {/* Skill rows */}
                    {skills.map((sk, i) => {
                      const isActive = activeSkillId === sk.id
                      return (
                        <button
                          key={sk.id}
                          onClick={() => setSkill(sk.id, sk.name, sk.collection)}
                          className="w-full flex items-start gap-2 px-3 py-2 text-xs cursor-pointer transition text-left"
                          style={{ borderBottom: i < skills.length - 1 ? '1px solid var(--border)' : 'none',
                                   background: isActive ? 'rgba(138,180,248,0.1)' : 'transparent',
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            ...
                            {sk.description && (
                              <div className="text-[10px] truncate mt-0.5 opacity-60">{sk.description}</div>
                            )}
                          </div>
                        </button>
                      )
