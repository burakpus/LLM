...
type Tab = 'upload' | 'documents' | 'skills' | 'usage' | 'settings'
    ...
    <div className="min-h-dvh flex flex-col" style={{ background: 'var(--bg)', color: 'var(--text)' }}>
      ...
      >
        ...
        <nav className="flex items-center gap-1">
          {(['upload', 'documents', 'skills', 'usage', 'settings'] as Tab[]).map(tb => (
            <button
              key={tb}
              onClick={() => setTab(tb)}
            ...
            >
              {tb === 'upload' ? 'Upload' : tb === 'documents' ? 'Documents' : tb === 'skills' ? 'Skills' : tb === 'usage' ? 'Kullanım' : '⚙ Ayarlar'}
            </button>
          ))}
        </nav>
      </header>
...
// =============================================================================
// Tab 5 — Settings (Connection + System Prompt — moved from SettingsPanel)
// =============================================================================
