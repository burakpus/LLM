...
export default function SqlDataDialog({ conn, onClose, onJobStarted }: Props) {
  ...
  const onAddGroup = async () => {
    ...
    try {
      await createTableGroup(conn.id, newGroupName.trim(), groups.length)
      setNewGroupName('')
      const grps = await listTableGroups(conn.id)
      setGroups(grps)
    } catch (e: any) { setErr(e.message) }
  }
              ...
              {groupBuckets.filter(b => b.items.length > 0).map(b => (
                <div key={`g-${b.id ?? 'na'}`} className="rounded-xl overflow-hidden"
                  ...
                  <div className="px-3 py-2 text-xs font-semibold flex items-center justify-between"
                    ...
                    <span>📁 {b.name} ({b.items.length})</span>
                  </div>
                  {b.items.map(t => {
                          ...
                          <div className="text-[10px]" style={{ color: 'var(--mute)' }}>
                            ...
                            {cfg && (
                              <>
                                ...
                                {cfg.lastSyncedAt && <span> · son sync: {new Date(cfg.lastSyncedAt).toLocaleDateString()}</span>}
                              </>
                            )}
                          </div>
                        </div>
